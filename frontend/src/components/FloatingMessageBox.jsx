...
export default function FloatingMessageBox({ auth }) {
  ...
  const loadMessages = async () => {
    try {
      ...
      // server returns pinned + others in created order (server already ordered pinned first),
      // but we will store arr where pinned messages still present and we render pinned separately.
  ...
  useEffect(() => {
    ...
    presenceIntervalRef.current = setInterval(() => {
      sendPresence("online");
      // prune stale onlineMap entries locally
      const now = Date.now();
      setOnlineMap((prev) => {
        const next = {};
    ...
    if (payload.type === "message:new") {
      ...
      if (!open) {
        ...
        if (window.Notification && Notification.permission === "granted") {
          new Notification(`${m.username}`, { body: m.content.slice(0, 120) });
        }
      } else {
        setUnread(0);
      }
    ...
    <>
      ...
      <button
        onClick={() => {
          setOpen((v) => !v);
          if (!open) setUnread(0);
          // when toggling open -> announce presence (fresh)
          sendPresence("online");
        }}
        className="fixed bottom-6 right-6 z-50 bg-orange-600 text-white w-16 h-16 rounded-full shadow-xl flex items-center justify-center text-2xl hover:bg-orange-700 transition"
        title="Messages"
      >
        💬
        {unread > 0 && (
      ...
      {/* Panel */}
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-96 h-[520px] bg-white shadow-2xl rounded-xl border border-gray-200 flex flex-col slide-up-panel overflow-hidden">
          {/* Header */}
          <div className="bg-orange-600 text-white p-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-white/80">{messages.filter(m => m.pinned).length} pinned • {onlineUsersList.length} online</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex -space-x-1">
                {onlineUsersList.slice(0,5).map(u => (
                  <div key={u} className="w-6 h-6 rounded-full ring-1 ring-white text-[10px] flex items-center justify-center text-white" title={u} aria-hidden>
                    <span className={colorFromName(u) + " w-6 h-6 flex items-center justify-center rounded-full text-xs"}>{initialsFromName(u)}</span>
                  </div>
                ))}
              </div>
          ...
          {/* Messages container */}
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 bg-gray-50 space-y-3">
            {/* pinned first (always shown at top of list) */}
            ...
            {/* non-pinned — messages are rendered in chronological order so newest at bottom */}
            {messages.filter(m => !m.pinned).map(m => (
          ...
          <div className="p-3 border-t flex gap-2 items-center">
            ...
            />
            <button onClick={sendMessage} className="bg-orange-600 hover:bg-orange-700 text-white px-3 py-1 rounded">Send</button>
          </div>
        </div>
      )}
    </>
...
function EmojiPicker({ onSelect }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(v => !v)} className="px-2 py-1 rounded border bg-white">😊</button>
      {open && (
        <div className="absolute bottom-10 left-0 bg-white border p-2 rounded shadow grid grid-cols-6 gap-1 z-50">
          {emojis.map(e => <button key={e} onClick={() => { onSelect(e); setOpen(false); }} className="text-lg">{e}</button>)}
        </div>
      )}
    </div>
  );
...
function MessageCard({ m, me, isAdmin, deleteMsg, togglePin, pinned }) {
  ...
  return (
    <div className={`flex w-full ${mine ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[78%] px-3 py-2 rounded-lg shadow-sm msg-enter
